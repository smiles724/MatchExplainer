...
class PGExplainer(Explainer):
    ...
    def __init__(self, device, gnn_model, n_in_channels=14, e_in_channels=3, hid=64, n_layers=2, ):
        super(PGExplainer, self).__init__(device, gnn_model)
        self.device = device
        self.edge_mask = EdgeMaskNet(n_in_channels, e_in_channels, hid=hid, n_layers=n_layers).to(self.device)
    ...
    def explain_graph(self, graph, model=None, temp=0.1, ratio=0.1, draw_graph=0, vis_ratio=0.2, train_mode=False,
                      supplement=False):
        ...
        if train_mode:
            # (1) batch version: get positive edge index(G_s) for ego g
