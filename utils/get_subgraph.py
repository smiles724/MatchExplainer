...
def get_neg_edge_index(g):
    neg_edge_index = batched_negative_sampling(edge_index=g.edge_index, batch=g.batch, num_neg_samples=None,
...
def get_broken_graph(g, broken_ratio, connectivity=True):
    ...
    out_edge_ratio = []
    broken_masks = []
    for edge_index, N, C, E in zip(edge_indices, num_nodes.tolist(), cum_nodes.tolist(), num_edges.tolist()):
        if connectivity:
            flag = 0
            ...
            for num_hops in range(1, MAX_DIAM):
                _, _, _, broken_mask = bid_k_hop_subgraph(node_idx=node_idx, num_hops=num_hops,
                    edge_index=edge_index - C, num_nodes=N)
...
# Bidirectional k-hop subgraph modified from torch-geometric.utils.subgraph
def bid_k_hop_subgraph(node_idx, num_hops, edge_index, relabel_nodes=False, num_nodes=None):
    r"""Computes the :math:`k`-hop subgraph of :obj:`edge_index` around node
    :attr:`subg_idx`.
    It returns (1) the nodes involved in the subgraph, (2) the filtered
    :obj:`edge_index` connectivity, (3) the mapping from node indices in
    :obj:`subg_idx` to their new location, and (4) the edge mask indicating
    which edges were preserved.

    Args:
        subg_idx (int, list, tuple or :obj:`torch.Tensor`): The central
            node(s).
