...
class Mutagenicity(InMemoryDataset):
    ...
    def raw_file_names(self):
        ...
        return ['Mutagenicity/' + i for i in
                ['Mutagenicity_A.txt', 'Mutagenicity_edge_labels.txt', 'Mutagenicity_graph_indicator.txt',
                    'Mutagenicity_graph_labels.txt', 'Mutagenicity_node_labels.txt', ]]
    ...
    def download(self):
        if os.path.exists(osp.join(self.raw_dir, 'Mutagenicity')):
            print('Using existing node_features in folder Mutagenicity')
            return
    ...
    def process(self):
        ...
        for i in range(num_graphs):
            ...
            data = Data(x=x[perm], y=y[i], z=node_label[perm], edge_index=edge_index[:, begin:end] - int(min(perm)),
                        edge_attr=edge_attr[begin:end], name="mutag_%d" % i, idx=i)
