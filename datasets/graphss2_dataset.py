...
def split(data, batch):
    ...
    # Edge indices should start at zero for every g.
...
def get_dataset(dataset_dir, dataset_name, task=None):
    sync_dataset_dict = {'BA_2Motifs'.lower(): 'BA_2Motifs', 'BA_Shapes'.lower(): 'BA_shapes',
        'BA_Community'.lower(): 'BA_Community', 'Tree_Cycle'.lower(): 'Tree_Cycle', 'Tree_Grids'.lower(): 'Tree_Grids',
        'BA_LRP'.lower(): 'ba_lrp'}
    ...
    def process(self):
        ...
        for i in range(1, 189):
            ...
            data_example = Data(x=torch.from_numpy(one_hot_feature).float(),
                                edge_index=dense_to_sparse(torch.from_numpy(adj))[0], y=label)
...
class SentiGraphDataset(InMemoryDataset):
    ...
    def process(self):
        # Read node_features into huge `Data` list.
...
class BA_LRP(InMemoryDataset):
    ...
    @property
    def processed_file_names(self):
        return [f'node_features{self.num_per_class}.pt']
