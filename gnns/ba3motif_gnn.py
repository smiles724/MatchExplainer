...
class BA3MotifNet(torch.nn.Module):
    ...
    @overload   # 自定义的decorator
    def forward(self, x, edge_index, edge_attr, batch):
        # node-level representations (N_b,D), where N_b is the sum of nodes in the batch
        node_x = self.get_node_reps(x, edge_index, edge_attr, batch)

        # graph-level representations, getting (B,D)
...
if __name__ == '__main__':
    ...
    torch.save(model.cpu(), osp.join(args.model_path, 'ba3_net.pt'))
