...
def segment_sum(data, segment_ids):
    ...
    :param data: A pytorch tensor of the node_features for segmented summation.
    :param segment_ids: A 1-D tensor containing the indices for the segmentation.
    :return: a tensor of the same type as node_features containing the results of the segmented summation.
    """
...
def unsorted_segment_sum(data, segment_ids, num_segments):
    ...
    :return: A tensor of same node_features type as the node_features argument.
    """

    assert all([i in data.shape for i in segment_ids.shape]), "segment_ids.shape should be a prefix of node_features.shape"
    ...
    # tensor = torch.zeros(num_segments, node_features.shape[1], device=node_features.device)
    # for index in range(num_segments):
    #     tensor[index, :] = torch.sum(node_features[segment_ids == index, :], dim=0)
    # return tensor
    ...
    assert data.shape == segment_ids.shape, "node_features.shape and segment_ids.shape should be equal"
